...
namespace soft20181_starter.Pages.Admin.Events
{
    ...
    public class DeleteSearchResultsModel : PageModel
    {
        ...
        public DeleteSearchResultsModel(ILogger<DeleteSearchResultsModel> logger, EventAppDbContext context)
        {
            _logger = logger;
            _context = context;
        }
        ...
        public async Task<IActionResult> OnGetAsync(string eventName, string location)
        {
            ...
            {
                _logger.LogWarning("Search for events to delete with no criteria provided");
                return RedirectToPage("SearchForDelete");
            }

            _logger.LogInformation("Searching for events to delete with Name: {EventName}, Location: {Location}", 
